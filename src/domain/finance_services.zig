...
pub fn data_summary(
    ...
    var category_breakdown: [num_categories]NumberType = .{0} ** category_info.@"enum".fields.len;

    // Totals for indiviual categories
    for(data) |datum| {
        const amount = @field(datum, amount_field);
        data_total += amount;
        const cat_index = std.meta.fieldIndex(DataType, select_datatype_field).?; // <- Would not compile if it didn't have the field
